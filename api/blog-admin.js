...
<body>
<!-- MOBILE MENU BUTTON -->
<button type="button" class="mobile-menu-btn" onclick="document.querySelector('.sidebar').classList.toggle('open')" aria-label="Menu">
...
<div style="position:relative;"><input type="password" class="login-input" id="pw-input" placeholder="Admin password" autocomplete="current-password" style="padding:14px 40px 14px 12px;font-size:15px;" onkeydown="if(event.key==='Enter'){doLogin();return false;}"><button type="button" onclick="togglePasswordVisibility('pw-input', this)" style="position:absolute;right:10px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;color:#4A7A7C;font-size:16px;">&#128065;<\/button><\/div>
...
<div id="reset-confirm-form" style="display:none;padding:24px;">
  ...
  <div style="position:relative;margin-bottom:12px;"><input type="password" id="new-password" placeholder="New password (min 8 chars)" style="width:100%;padding:14px 40px 14px 12px;font-size:15px;border:1px solid #ccc;border-radius:6px;box-sizing:border-box;"><button type="button" onclick="togglePasswordVisibility('new-password', this)" style="position:absolute;right:10px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;color:#4A7A7C;font-size:16px;">&#128065;<\/button><\/div>
  <div style="position:relative;margin-bottom:12px;"><input type="password" id="confirm-password" placeholder="Confirm new password" style="width:100%;padding:14px 40px 14px 12px;font-size:15px;border:1px solid #ccc;border-radius:6px;box-sizing:border-box;"><button type="button" onclick="togglePasswordVisibility('confirm-password', this)" style="position:absolute;right:10px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;color:#4A7A7C;font-size:16px;">&#128065;<\/button><\/div>
...
<button type="button" class="nav-item active" id="nav-posts" onclick="showView('posts')">
...
<\/button>
<button type="button" class="nav-item" id="nav-new" onclick="newPost()">
<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125"/><\/svg>
New Post
<\/button>
<button type="button" class="nav-item" id="nav-drafts" onclick="showView('drafts')">
...
<button type="button" class="nav-item" id="nav-images" onclick="showView('images')">
<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m2.25 15.75 5.159-5.159a2.25 2.25 0 0 1 3.182 0l5.159 5.159m-1.5-1.5 1.409-1.409a2.25 2.25 0 0 1 3.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 0 0 1.5-1.5V6a1.5 1.5 0 0 0-1.5-1.5H3.75A1.5 1.5 0 0 0 2.25 6v12a1.5 1.5 0 0 0 1.5 1.5Zm10.5-11.25h.008v.008h-.008V8.25Zm.375 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Z"/><\/svg>
Image Library
<\/button>
<button type="button" class="nav-item" id="nav-settings" onclick="showView('settings')">
...
Settings
<\/button>
<button type="button" class="nav-item" id="nav-batch" onclick="showView('batch')">
<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M20.25 7.5l-.625 10.632a2.25 2.25 0 0 1-2.247 2.118H6.622a2.25 2.25 0 0 1-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125Z"/><\/svg>
Batch Upload
<\/button>
<\/div>
<div class="sidebar-bottom">
<button type="button" class="logout-btn" onclick="doLogout()">Sign out<\/button>
<\/div>
<\/nav>
<!-- Main area -->
...
<button type="button" class="btn-new-post" onclick="newPost()">+ New Post<\/button>
<\/div>
...
<div id="faq-list"><\/div>
<button type="button" class="btn-add-faq" onclick="addFaqItem()">+ Add FAQ<\/button>
<\/div>
<div class="field-group" style="margin-top:20px">
<div class="field-label">Keep Reading (Related Posts)<\/div>
<div id="related-list"><\/div>
<button type="button" class="btn-add-faq" onclick="addRelatedRow()">+ Add Related Post<\/button>
<\/div>
<div class="field-group">
...
<button type="button" class="btn-choose-hero" style="flex:1" onclick="openImageManager('hero')">Choose from Library<\/button>
<button type="button" class="btn-choose-hero" style="flex:1" onclick="openHeroUpload()">Upload New<\/button>
<\/div>
...
<button type="button" class="btn-delete-post" id="btn-delete-post" onclick="confirmDelete()" style="display:none">Delete post<\/button>
<\/div>
<div class="action-bar-right">
<button type="button" class="btn-cancel-edit" onclick="cancelEdit()">Cancel<\/button>
<button type="button" class="btn-save-draft" id="btn-save-draft" onclick="saveDraft()">Save Draft<\/button>
<button type="button" class="btn-preview-post" onclick="previewPost()">Preview<\/button>
<button type="button" class="btn-schedule" id="btn-schedule" onclick="openScheduleModal()" style="display:none">Schedule<\/button>
<button type="button" class="btn-publish" id="btn-publish" onclick="goLive()">Go Live<\/button>
<\/div>
<\/div>
<\/div>
...
<button type="button" id="btn-img-select" class="btn-save-draft" onclick="enterSelectMode()">Select<\/button>
<button type="button" id="btn-img-upload" class="btn-new-post" onclick="openImageManager('browse')">+ Add Photos<\/button>
<\/div>
<\/div>
...
<button type="button" class="btn-modal-cancel" onclick="exitSelectMode()">Cancel<\/button>
<button type="button" id="btn-delete-selected" class="btn-modal-confirm" disabled onclick="confirmMultiDelete()">Delete Selected (0)<\/button>
<\/div>
<\/div>
...
<button type="button" class="toggle-opt active" id="rt-auto-btn" onclick="setReadTimeMode('auto')">Auto (calculated from word count)<\/button>
<button type="button" class="toggle-opt" id="rt-manual-btn" onclick="setReadTimeMode('manual')">Manual<\/button>
<\/div>
<\/div>
<\/div>
<div class="settings-section">
...
<\/div>
<\/div>
<button type="button" class="btn-save-settings" onclick="saveSettings()">Save settings<\/button>
...
<\/div>
<button type="button" class="btn-modal-cancel" onclick="resetBatch()">&#8592; Back<\/button>
<\/div>
<div id="batch-preview-list"><\/div>
<div style="margin-top:16px">
<button type="button" class="btn-publish" id="btn-batch-upload" onclick="executeBatchUpload()" style="width:100%;padding:14px;font-size:14px">Upload &amp; Save as Drafts<\/button>
<\/div>
<\/div>
...
<button type="button" class="btn-new-post" onclick="showView('drafts')" style="margin-bottom:12px">Go to Drafts<\/button>
<\/div>
<div id="batch-complete-results" style="padding:0 4px"><\/div>
<div style="margin-top:12px">
<button type="button" class="btn-modal-cancel" onclick="resetBatch()">Upload another ZIP<\/button>
<\/div>
<\/div>
<\/div>
<\/main>
...
<button type="button" class="btn-modal-cancel" onclick="closeModal('delete-modal')">Cancel<\/button>
<button type="button" class="btn-modal-confirm" id="confirm-delete-btn" onclick="executeDelete()">Delete<\/button>
<\/div>
<\/div>
<\/div>
...
<button type="button" class="btn-modal-cancel" onclick="closeModal('img-in-use-modal')">Cancel<\/button>
<button type="button" class="btn-modal-confirm" onclick="deleteInUseAnyway()" style="background:#C82030">Delete Anyway<\/button>
<button type="button" class="btn-modal-stay" onclick="moveToUsedPhotos()">Move to Used Photos<\/button>
<\/div>
<div class="modal-actions" id="img-in-use-multi-actions" style="display:none">
<button type="button" class="btn-modal-cancel" onclick="closeModal('img-in-use-modal')">Cancel<\/button>
<button type="button" class="btn-modal-confirm" onclick="deleteMultiAnyway()" style="background:#C82030">Delete Anyway<\/button>
<button type="button" class="btn-modal-stay" onclick="moveMultiToUsedPhotos()">Move to Used Photos<\/button>
<\/div>
<\/div>
<\/div>
...
<div class="modal-actions">
<button type="button" class="btn-modal-leave" onclick="leaveWithoutSaving()">Leave without saving<\/button>
<button type="button" class="btn-modal-stay" onclick="closeModal('unsaved-modal')">Stay and keep editing<\/button>
<\/div>
<\/div>
<\/div>
...
<div class="modal-actions">
<button type="button" class="btn-modal-stay" onclick="closeModal('unsaved-uploads-modal')">Stay<\/button>
<button type="button" class="btn-modal-leave" onclick="forceCloseImageManager()">Leave without uploading<\/button>
<\/div>
<\/div>
<\/div>
...
<\/div>
<div class="modal-actions">
<button type="button" class="btn-modal-cancel" onclick="closeModal('schedule-modal')">Cancel<\/button>
<button type="button" class="btn-cancel-schedule" id="btn-cancel-schedule" onclick="cancelSchedule()" style="display:none">Remove schedule<\/button>
<button type="button" class="btn-schedule-confirm" onclick="confirmSchedule()">Schedule<\/button>
<\/div>
<\/div>
<\/div>
...
<button type="button" class="btn-upload-img" onclick="triggerUpload()">+ Add Photos<\/button>
<button type="button" class="btn-close-img" onclick="closeImageManager()">&times;<\/button>
<button type="button" class="btn-modal-cancel" id="img-picker-cancel" style="display:none;margin-right:8px" onclick="forceCloseImageManager()">Cancel<\/button>
<\/div>
<\/div>
...
<div class="modal-overlay" id="video-modal" style="display:none">
  <div class="modal-box">
    <div class="modal-header" style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
      ...
      <button type="button" onclick="closeVideoModal()" style="background:none;border:none;font-size:18px;color:#8AACAE;cursor:pointer;padding:0">&times;<\/button>
    <\/div>
    ...
    <div style="display:flex;gap:8px;justify-content:flex-end">
      <button type="button" class="btn-modal-cancel" onclick="closeVideoModal()">Cancel<\/button>
      <button type="button" class="btn-publish" onclick="insertVideo()">Insert<\/button>
    <\/div>
  <\/div>
<\/div>
