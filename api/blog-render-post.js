...
const CTA_TEXT = {
  ...
  wdw: 'Planning a Walt Disney World trip is complex. Theme Park Co-Pilot makes it simple â personalized day plans, live wait times, and park strategies built for your family.',
...
function ctaHtml(cta, park) {
  ...
  const btnText = cta.buttonText || 'Try free for 7 days â';
...
if (!related || !related.length) return '';
  var cards = related.map(function(r) {
    ...
    var rLabel = rPark === 'dl' ? 'Disneyland' : rPark === 'wdw' ? 'Walt Disney World' : 'Both Parks';
    var imgHtml = r.heroImage ? '<div class="related-img"><img src="' + esc(r.heroImage) + '" alt="' + esc(r.title) + '" loading="lazy"></div>' : '<div class="related-img related-img--empty"></div>';
    return '<a href="https://themeparkcopilot.com/blog/' + esc(r.slug) + '" class="related-card">' + imgHtml + '<div class="related-card-body"><span class="related-park">' + esc(rLabel) + '</span><span class="related-title">' + esc(r.title) + '</span><span class="related-link">Read guide \u2192</span></div></a>';
  }).join('');
  return '<div class="related-posts"><h3>You might also like</h3><div class="related-grid">' + cards + '</div></div>';
}

function breadcrumbHtml(post) {
  return '<nav class="breadcrumb"><a href="https://themeparkcopilot.com">Home</a><span> \u203a </span><a href="https://themeparkcopilot.com/blog">Blog</a><span> \u203a </span><span>' + esc(post.title) + '</span></nav>';
}
...
function breadcrumbSchema(post) {
  return JSON.stringify({
    ...
    '@type': 'BreadcrumbList',
    'itemListElement': [
      { '@type': 'ListItem', 'position': 1, 'name': 'Home', 'item': 'https://themeparkcopilot.com' },
      { '@type': 'ListItem', 'position': 2, 'name': 'Blog', 'item': 'https://themeparkcopilot.com/blog' },
      { '@type': 'ListItem', 'position': 3, 'name': post.title, 'item': 'https://themeparkcopilot.com/blog/' + (post.slug || '') }
      ]
  });
}
...
export default async function handler(req, res) {
  ...
  // Fix 6: Rate limiting â 100 requests per IP per minute
...
// Show updated date if post was updated more than 7 days after publish
var showUpdated = post.updatedAt && post.publishedAt &&
  (new Date(post.updatedAt) - new Date(post.publishedAt)) > 7 * 24 * 60 * 60 * 1000;

var pageUrl = 'https://themeparkcopilot.com/blog/' + esc(post.slug);
  var pageTitle = esc(post.title) + ' â Theme Park Co-Pilot';
...
var articleSchema = JSON.stringify({
  ...
  publisher: { '@type': 'Organization', name: 'Theme Park Co-Pilot', url: 'https://themeparkcopilot.com', logo: { '@type': 'ImageObject', url: 'https://app.themeparkcopilot.com/assets/brand/favicon.PNG' } },
  mainEntityOfPage: { '@type': 'WebPage', '@id': 'https://themeparkcopilot.com/blog/' + (post.slug || '') },
  keywords: (post.tags || []).join(', ')
});

var shareUrl = encodeURIComponent(pageUrl);
  var shareTitle = encodeURIComponent((post.title || '') + ' â Theme Park Co-Pilot');
...
var faqSchemaTag = (post.faqs && post.faqs.length) ? '<script type="application/ld+json">' + faqSchema(post.faqs) + '<\/scr' + 'ipt>' : '';
  var breadcrumbSchemaTag = '<script type="application/ld+json">' + breadcrumbSchema(post) + '<\/scr' + 'ipt>';

var updatedHtml = showUpdated ? '<span class="post-meta-sep">&middot;</span><span class="post-updated">Updated ' + formatDate(post.updatedAt) + '</span>' : '';

var html = '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>' + pageTitle + '</title><meta name="description" content="' + esc(post.metaDescription || post.intro || '') + '"><meta property="og:title" content="' + esc(post.title) + '"><meta property="og:description" content="' + esc(post.metaDescription || post.intro || '') + '"><meta property="og:type" content="article"><meta property="og:site_name" content="Theme Park Co-Pilot"><meta property="og:url" content="' + pageUrl + '"><meta property="og:image" content="' + ogImg + '"><meta property="article:published_time" content="' + esc(post.publishedAt || '') + '"><meta property="article:modified_time" content="' + esc(post.updatedAt || '') + '"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="' + esc(post.title) + '"><meta name="twitter:description" content="' + esc(post.metaDescription || post.intro || '') + '"><meta name="twitter:image" content="' + ogImg + '"><meta name="keywords" content="' + esc((post.tags || []).join(', ')) + '"><link rel="canonical" href="' + pageUrl + '"><link rel="sitemap" type="application/xml" href="/sitemap.xml"><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Fraunces:ital,opsz,wght@0,9..144,900;1,9..144,900&family=Outfit:wght@400;600;700;800&display=swap" rel="stylesheet"><link rel="icon" href="https://app.themeparkcopilot.com/assets/brand/favicon.PNG"><style>' + POST_CSS + '</style><script type="application/ld+json">' + articleSchema + '<\/scr' + 'ipt>' + faqSchemaTag + breadcrumbSchemaTag + '</head><body>'
...
// Fix 4: Content Security Policy for public pages
