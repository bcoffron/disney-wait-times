...
function sortPosts(posts) {
  ...
  // Add any posts not in the curated order at the end
...
function renderCard(post, isFeatured) {
  var slug = esc(post.slug), title = esc(post.title), intro = esc(post.intro || post.metaDescription || '');
  ...
  var catLabel = esc(post.category || '');

  if (isFeatured) {
    return '<a class="post-card post-card--featured" href="/blog/' + slug + '" data-category="' + cat + '" aria-label="' + title + '"><div class="post-card__img-wrap"><img src="' + heroImage + '" alt="' + heroAlt + '" class="post-card__img" loading="eager"><span class="post-card__badge">START HERE</span></div><div class="post-card__body"><div class="post-card__meta"><span class="post-tag ' + tClass + '"' + tStyle + '>' + tagLabel + '</span><span class="post-card__category">' + catLabel + '</span></div><h2 class="post-card__title">' + title + '</h2><p class="post-card__intro">' + intro + '</p></div></a>';
  }
...
export default async function handler(req, res) {
  ...
  + '<script>(function(){var pills=document.querySelectorAll(".filter-pill");var cards=document.querySelectorAll(".post-grid [data-category]");pills.forEach(function(pill){pill.addEventListener("click",function(){var filter=this.getAttribute("data-filter");pills.forEach(function(p){p.classList.remove("active");});this.classList.add("active");cards.forEach(function(card){if(filter==="all"||card.getAttribute("data-category").indexOf(filter)!==-1){card.style.display="";}else{card.style.display="none";}});});});})();</script>'
  + '</body></html>';
