...
Return ONLY valid JSON: {"sections":[{"title":"string","entries":[{"t":"H:MM AM/PM","name":"string","type":"ride|show|dining|break","note":"string","land":"string"}]}],"explanation":"string"}
No markdown, no extra text. Valid JSON only.`;
    ...
    const r = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        system: systemPrompt,
        tools: tools,
        messages: [{ role: 'user', content: 'Optimize this ' + dayLabel + ' Disneyland schedule:\n' + schedule }]
      })
    });
    ...
    if (!match) return res.status(500).json({ error: 'No JSON in response', raw: text.substring(0, 200) });
