...
body:JSON.stringify({model,max_tokens:2000,system:systemPrompt||"You are an expert Disneyland touring planner.",tools,messages:[{role:"user",content:prompt}]})
});
...
if(jm){
try{const sections=JSON.parse(jm[1].trim());return res.status(200).json({sections,explanation:sm?sm[1].trim():""});}
